...
export function checkLineBarrierXMinus(matrix, x1, y1, x2, y2, target) {
  if (matrix && x1 && y1 && x2 && y2 && target) {
    ...
    let xm = x1 + target;
    // debugger;
    const qwewq = checkLineY(matrix, pMinY.x, xm, pMinY.y);
    const asdas = checkLineX(matrix, pMinY.y, pMaxY.y, xm);
    const asdsa = checkLineY(matrix, xm, pMaxY.x, pMaxY.y);
    debugger;
    if (checkLineY(matrix, pMinY.x, xm, pMinY.y)) {
      if (
        checkLineX(matrix, pMinY.y, pMaxY.y, xm) &&
